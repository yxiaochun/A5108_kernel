...
/* MALI_SEC_INTEGRATION */
		kbdev->pm.callback_power_dvfs_on = callbacks->power_dvfs_on_callback;
		kbdev->pm.callback_power_change_dvfs_level = callbacks->power_change_dvfs_level_callback;
	} else {
...
/* MALI_SEC_INTEGRATION */
		kbdev->pm.callback_power_dvfs_on = NULL;
		kbdev->pm.callback_power_change_dvfs_level = NULL;
	}
...
mali_error kbase_pm_powerup(struct kbase_device *kbdev)
{
	...
	ret = kbase_pm_init_hw(kbdev, MALI_FALSE, MALI_FALSE);
